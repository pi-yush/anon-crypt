...
def calc_entropy(i,pf,sample_size,N,all_entropy,outdegree,max_hops,all_frac_captured):
#for i in range(10,C):
...
if __name__ == '__main__':
    ...
    for i in range(startc,endc,increment):
        ...
        p = mp.Process(target=calc_entropy,args=(adv_nodes,pf,sample_size,i,all_entropy,outdegree,max_hops,all_frac_captured))
        processes.append(p)
        p.start()
    ...
    json.dump(all_entropy.copy(),open('Dan++_vary_N_' + str(datetime.date.today()) + '_' + str(datetime.datetime.now().hour) + '.txt','w'))
    ...
    plt.savefig('Dan++_vary_N_scaled' + str(datetime.date.today()) + '_' + str(datetime.datetime.now().minute), dpi=600)
    #plt.show()
