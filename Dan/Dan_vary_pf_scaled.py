...
def calc_entropy(i,G,sample_size,N,outdegree,pf,all_entropy,all_frac_captured):
    '''Function that calculates the entropy for given parameters'''
    entropy = []
    adversaries = 10
    adv_combinations = []

    # Generating sample_size number of adversarial combinations
    ...
    # Calculating entropy for each set of adversary nodes in the network
    for j in adv_combinations:
        #print(j)
        G1 = deepcopy(G)
        rem_edges = []

        # Removing all outgoing edges of the adversary nodes to make sure that
        # a transaction is captured and considered by the first adverdary node. 
        for k in j:
        ...
        # Entropy computation for transactions captured by each adversarial node
        for adv_node in j:
            adv_count +=1
            prob = []
            norm_factor = 0.0
            ...
            for node in honest_nodes:
                ...
                for path in paths_to_adv:
                    ...
                    for index in range(0,len(path)-2):
                        path_val = path_val*(1/outdegree)*i
                    int_expr+= path_val
        ...
        frac_captured.append((total_tran_captured*100)/(1000*(N-adversaries)))
    
    entropy.append(adv_combination_entropy)
    all_entropy[i] = adv_combination_entropy
    all_frac_captured[i] = sum(frac_captured)/len(frac_captured)
    # plt.boxplot(entropy)
    # plt.savefig('Dandelion/entropy'+str(N)+'_'+str(i)+'_'+str(sample_size)+'_'+str(outdegree))
    # plt.figure()               
    #nx.draw(G)
    #plt.show()
...
if __name__ == '__main__':
    ...
    init = time.time()
    processes = []
    for i in np.arange(startc,endc,increment):
        i = round(i,1)
        p = mp.Process(target=calc_entropy,args=(i,G,sample_size,N,outdegree,pf,all_entropy,all_frac_captured))
        processes.append(p)
        p.start()
    ...
    #json.dump(all_entropy.copy(),open('Dan_vary_adversary_fraction_scaled' + str(N) + '_' + str(datetime.date.today()) + '_' + str(datetime.datetime.now().hour) + '.txt','w'))
    ...
    plt.yticks(range(0,10), weight = 'bold', fontsize=12)
    ax = plt.gca()
    ax.set_xlabel('Forwarding Probability (pf)', fontsize='x-large', fontweight='bold')
    ax.set_ylabel('Entropy', fontsize='x-large', fontweight='bold')
    plt.savefig('Dan_vary_pf' + str(N) + '_' + str(datetime.date.today()) + '_' + \
