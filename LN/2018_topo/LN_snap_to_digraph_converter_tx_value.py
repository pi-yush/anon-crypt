...
def build_graph_from_snapshot(new, tx_val):
    ...
    SNAPSHOT_DIR = './' # make sure to point this to the directory that contains all the LN snapshot files
    if new:
    ...
    #Extracting node identifiers and mapping them to numbers to save memory and computation
    ...
    if new:
        edges = [(nodemap[edge['nodes'][0]], nodemap[edge['nodes'][1]],
            ...
            'direction': '1'#,
            #'capacity' : edge['satoshis']/2 # Adding capacity info in the graph
            }
    ...
    ebf = []
    efpm = []
    hashed_policy = {}#{e['short_channel_id']: e for e in policy}
    ...
    #Calculating individual channel weights
    weight = []
    for edge in edges:    
    ...
    #Creating LN graphs from nodes and edges information extracted from real LN snapshots
    G = nx.MultiGraph() 
    ...
    #Extracting the largest connected component to be used for the analysis
