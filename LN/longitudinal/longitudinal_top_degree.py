...
from longitudinal_LN_snap_to_digraph_converter import build_graph_from_snapshot
...
if __name__ == '__main__':
    ...
    G,node_list,snap_name = build_graph_from_snapshot(True, False)
    ...
    if(os.path.isfile(snap_name + 'dj.pkl')):
        with open(snap_name + 'dj.pkl', 'rb') as dj_paths:
            path = pickle.load(dj_paths)
    else:
        path = dict(nx.all_pairs_dijkstra_path(G))
        with open(snap_name + 'dj.pkl', 'wb') as dj_paths:
            pickle.dump(path,dj_paths)

    print('Calculated the shortest paths!')
    sample_size = 1
    adv_nodes_count = int(nodes/100)
    interval_dir = snap_name + 'top_degree' + '_' + str(adv_nodes_count) + '/'
    #nodes_affected_overall = {}
    calc_entropy(0,G,adv_nodes_count,nodes_affected_overall,entropy,\
             path,node_list,nodes,interval_dir, node_deg)
